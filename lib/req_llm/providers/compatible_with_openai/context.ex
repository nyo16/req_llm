defmodule ReqLLM.Providers.CompatibleWithOpenAI.Context do
  @moduledoc false
  defstruct [:context]
  @type t :: %__MODULE__{context: ReqLLM.Context.t()}
end

# Protocol implementation for CompatibleWithOpenAI-specific context encoding
# This reuses the exact same logic as OpenAI since it's the same API format
defimpl ReqLLM.Context.Codec, for: ReqLLM.Providers.CompatibleWithOpenAI.Context do
  ...
  # Private helpers - reuse OpenAI encoding logic
  defp encode_message(%ReqLLM.Message{role: role, content: parts}) do
    %{
      "role" => role_to_openai_role(role),
  ...
  defp role_to_openai_role(:tool), do: "tool"
  defp role_to_openai_role(role), do: to_string(role)
